...
class EighteenSOutputTables(EighteenSBase):
    ...
    def make_and_write_raw_abund_output_table(self):
        ...
        ).to_csv(os.path.join(self.output_dir, 'raw_seq_abund.csv'), index=True, columns=True)
    ...
    def make_and_write_tax_output_table(self):
        pd.DataFrame.from_dict(
            ...
            orient='index',
            columns=self.master_seq_abund_order_list
        ).to_csv(os.path.join(self.output_dir, 'tax_annotation.csv'), index=True, columns=True)
    ...
    def make_and_write_consolidated_host_output_table(self):
        ...
        ).to_csv(os.path.join(self.output_dir, 'consolidated_host.csv'))
    ...
    def _populated_consolidated_df_dict(self):
        ...
        for sample_name in self.info_df.index:
            ...
            consol_seq_to_orig_seq_list = defaultdict(list)

            for i in range(0, len(fasta_file), 2):
                seq = fasta_file[i+1]
                try:
                    # If there is a representative seq for this seq, then log it
                    consol_seq_to_orig_seq_list[coral_blasted_seq_to_consolidated_seq_dict[seq]].append(seq)
                except KeyError:
                    # else just map it to its self in the dict
                    consol_seq_to_orig_seq_list[seq].append(seq)
            ...
            for master_consolidated_seq in self.host_only_master_seq_abund_order_list:
                if master_consolidated_seq in consolidated_host_seqs_abund_dict:
                    # Get the summation of the abundances for the represented sequences
                    temp_abund_list.append(sum([name_rel_abund_dict[repped_seq] for repped_seq in consol_seq_to_orig_seq_list[master_consolidated_seq]]))
                else:
